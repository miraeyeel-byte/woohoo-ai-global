    ...
    if st.session_state.confirm_buy_cost > 0:
        ...
        with c1: 
            if st.button(f"{T('pull_1')} (0.01 SOL)", key="gp1"): 
                st.session_state.confirm_buy_cost = 0.01; st.session_state.confirm_buy_count = 1; st.rerun()
        with c2: 
            if st.button(f"{T('pull_5')} (0.05 SOL)", key="gp5"): 
                st.session_state.confirm_buy_cost = 0.05; st.session_state.confirm_buy_count = 5; st.rerun()
        with c3: 
            if st.button(f"{T('pull_10')} (0.10 SOL)", key="gp10"): 
                st.session_state.confirm_buy_cost = 0.10; st.session_state.confirm_buy_count = 10; st.rerun()
        with c4:
            if st.button(f"{T('pull_100')} (1.00 SOL)", key="gp100", type="primary"): 
                st.session_state.confirm_buy_cost = 1.00; st.session_state.confirm_buy_count = 100; st.rerun()
